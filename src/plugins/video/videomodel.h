...
public:
    ...
    struct Node {
        enum Type { RootNode, MovieNode, DotDot };
    ...
    static bool movieLessThan(Node *n1, Node *n2);
...
private:
    ...
    void updateMovie(const QSqlRecord &record, Node *parentNode);

    QModelIndex indexForNode(VideoModel::Node *node) const;
...
class VideoModelImageProvider : public QDeclarativeImageProvider
{
public:
    VideoModelImageProvider();
    ~VideoModelImageProvider();

    QImage requestImage(const QString &id, QSize *size, const QSize &requestedSize);
};

QML_DECLARE_TYPE(VideoModel)
